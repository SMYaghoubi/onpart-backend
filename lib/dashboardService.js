...
async function buildDashboard(db, now = new Date()) {
  ...
  const queries = [
    ...
    rowsWithFallback(db, 'order_activities', "SELECT 'order' type,o.id entity_id,CONCAT('سفارش #',o.id) title,o.created_at,'/admin/orders.html' link FROM orders o ORDER BY o.created_at DESC LIMIT 12", null, warnings),
    rowsWithFallback(db, 'payment_activities', "SELECT 'payment' type,p.id entity_id,CONCAT('پرداخت #',p.id) title,p.created_at,'/admin/payments.html' link FROM payments p ORDER BY p.created_at DESC LIMIT 12", null, warnings),
    rowsWithFallback(db, 'supplier_activities', "SELECT 'supplier' type,b.id entity_id,CONCAT('درخواست تأمین‌کننده #',b.id) title,b.submitted_at created_at,'/admin/supplier-updates.html' link FROM supplier_update_batches b ORDER BY b.submitted_at DESC LIMIT 12", null, warnings),
