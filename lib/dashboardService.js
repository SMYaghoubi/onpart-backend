...
const ACTIVE_SHOP_USERS_SQL = "SELECT COUNT(*) active_users FROM users WHERE role='user' AND status='active'";
...
async function buildDashboard(db, now = new Date()) {
  ...
  const queries = [
    ...
    rowsWithFallback(db, 'active_users', ACTIVE_SHOP_USERS_SQL, null, warnings),
  ...
  return {
    metrics: {
      ...
      active_users: usersCount[0]?.active_users == null ? null : Number(usersCount[0].active_users),
...
module.exports = { buildDashboard, mergeActivities, rowsWithFallback, ACTIVE_SHOP_USERS_SQL };