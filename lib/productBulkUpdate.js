const { parseAvailability } = require('./productAvailability');
const FIELD_RULES=Object.freeze({
  ...
  available:value=>parseAvailability(value).available,
  has_flow:value=>binaryFlag(value,'گردش'),
  supplier_id:value=>value===null||value===''?null:positiveInteger(value,'تأمین‌کننده')
});

function binaryFlag(value,label){
  if(value===true||value===1||value==='1')return 1;
  if(value===false||value===0||value==='0')return 0;
  throw new Error(`${label} فقط باید «دارد» یا «ندارد» باشد`);
}
