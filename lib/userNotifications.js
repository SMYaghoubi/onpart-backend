...
async function createUserNotification(userId, title, body, type = 'info', link = '/orders', soundKey = null, entityType = null, entityId = null) {
  try {
    if (!userId) return;
    await db.execute(
    ...
    if (err.code === 'ER_BAD_FIELD_ERROR' || err.errno === 1054) {
      try {
        try {
          await db.execute(
            `INSERT INTO user_notifications (user_id,title,body,type,link,sound_key)
             VALUES (?,?,?,?,?,?)`,
            [userId, String(title).slice(0,180), String(body).slice(0,2000), type, link, soundKey]
          );
        ...
        console.error('Create legacy user notification failed:', legacyErr.message);
        return;
      }
    }
...
async function deleteUserNotificationsForEntity(connection, userId, entityType, entityId, legacyLink = null) {
  ...
  try {
    ...
    // Allow entity deletion while migration 007 is being rolled out to an older DB.
    if ((err.code !== 'ER_BAD_FIELD_ERROR' && err.errno !== 1054) || !legacyLink) throw err;
    const [result] = await connection.execute(
      `DELETE FROM user_notifications
       WHERE user_id=? AND link=? AND (title LIKE ? OR body LIKE ?)`,
      [userId, legacyLink, `%#${Number(entityId)}%`, `%#${Number(entityId)}%`]
    );
    return Number(result.affectedRows) || 0;
  }
}
...
function broadcastUserDataChanged(entity = 'account', action = 'updated') {
  broadcast('user-data-changed', { entity, action });
}

module.exports = { createUserNotification, deleteUserNotificationsForEntity, broadcastUserNotificationsChanged, broadcastUserDataChanged };
